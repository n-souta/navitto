...
| 機能 | 状態 |
|------|------|
| メタボックスにアイコン選択UI（アイコンピッカー） | ✅ 完了 |
| 見出しごとのアイコン保存（_navitto_h2_icons） | ✅ 完了 |
| フロントエンドでのアイコン表示 | ✅ 完了 |
| Font Awesome 同梱（nv セット） | ✅ 完了 |

※ ライセンス検証は未実装のため、現時点では全ユーザーが利用可能。有料化時は Phase 7-B で制限予定（`docs/spec-product.md` 参照）。
...
- **有料化対応:** ライセンスキー検証（Phase 7-B）、販売準備（Phase 8）、WordPress.org 申請（Phase 9）。製品・価格・工程の詳細は `docs/spec-product.md` を参照。
- 公式サイト・Pro 版は後回し（`docs/spec-product.md` の Phase 10〜11）
