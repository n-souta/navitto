...
- **feature/icon-picker と feature/hide-nav-after-last-h2 のマージ方針確定・実施**
  - カスタム項目を残すか削除するかを決め、マージ順・コンフリクト対応を決定する。
  - 決めた順で main（または develop）にマージし、動作・表示の確認を行う。
- リリース: バージョン・CHANGELOG・タグ・GitHub Release を「5. リリース準備」「6. リリース」に沿って実施（両方マージする場合は v1.1.0 想定）。
