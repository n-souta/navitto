...
### 価格設定
- **無料版:** ¥0（永久無料）。目次連携・H2選択・表示開始位置・プリセット2種・横スクロール/均等割。
- **有料版（アイコン機能）:** ¥1,000（買い切り）を予定。無料版＋アイコン選択機能。詳細は `docs/spec-product.md` を参照。
- **将来 Pro 版:** 価格未定（検討中）。詳細は `docs/spec-product.md` を参照。
...
### Phase 7: アイコン機能 - **完了**
- メタボックスにアイコン選択UI、見出しごとのアイコン保存、フロント表示、Font Awesome 同梱。詳細は `docs/spec-product.md` を参照。

### Phase 7-B 以降: ライセンス検証・販売準備・WordPress.org 申請
- ライセンスキー検証（Gumroad 連携）、販売準備、WordPress.org 申請・公開。詳細は `docs/spec-product.md` を参照。
